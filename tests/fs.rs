...
#[cfg(unix)]
use filesystem_rs::UnixFileSystem;
use filesystem_rs::{DirEntry, FileSystem, OsFileSystem, TempDir, TempFileSystem};
...
macro_rules! make_test {
    ($test:ident, $fs:expr) => {
        ...
        fn $test() {
            ...
            let str = local.format("%Y-%m-%d_%H-%M-%S-%f").to_string();
            let mut path_custom_test = path.join(Path::new(&str));

            while (path_custom_test.exists()) {
                let local: DateTime<Local> = Local::now();
                let str = local.format("%Y-%m-%d_%H-%M-%S-%f").to_string();
                path_custom_test = path.join(Path::new(&str));
            }
            let _unused = fs.create_dir(&path_custom_test);

            super::$test(&fs, &path_custom_test);

            let _unused = fs.remove_dir_all(&path_custom_test);
        }
    };
}
...
macro_rules! test_fs {
    ($name:ident, $fs:expr) => {
        mod $name {
            ...
            make_test!(newly_created_directory_has_correct_timestamps, $fs);
            make_test!(newly_created_file_has_correct_timestamps, $fs);
            make_test!(timestamps_has_less_than_second_resolution, $fs);
...
fn remove_dir_fails_if_dir_is_not_empty<T: FileSystem>(fs: &T, parent: &Path) {
    ...
    // io::ErrorKind::DirectoryNotEmpty is not in stable rust, hence srv cannot raise it.
    // Instead Uncategorized is returned.
    // assert_eq!(result.unwrap_err().kind(), ErrorKind::DirectoryNotEmpty);
    assert_eq!(result.unwrap_err().kind(), ErrorKind::Uncategorized);
...
fn newly_created_directory_has_correct_timestamps<T: FileSystem>(fs: &T, parent: &Path) {
    let path = parent.join("directory");
    let result = fs.create_dir(&path);
    assert!(result.is_ok());

    // Server is on the same machine as client, timestamps should be very similar
    let meta = fs.metadata(&path);

    let accessed = meta.accessed().unwrap().elapsed().unwrap();
    assert!(
        accessed <= Duration::from_secs(1),
        "accessed elapsed time {}",
        accessed.as_secs()
    );
...
fn newly_created_file_has_correct_timestamps<T: FileSystem>(fs: &T, parent: &Path) {
    let path = parent.join("file");
    let result = fs.create_file(&path, "hello");
    assert!(result.is_ok());
...
fn timestamps_has_less_than_second_resolution<T: FileSystem>(fs: &T, parent: &Path) {
    let path_a = parent.join("file_a");
    let result = fs.create_file(&path_a, "");
    assert!(result.is_ok(), "File A creation result {:?}", result);

    // Wait a little before creating another file
    const SLEEP: u64 = 10;
    thread::sleep(Duration::from_millis(SLEEP));

    let path_b = parent.join("file_b");
    let result = fs.create_file(&path_b, "");
    assert!(result.is_ok(), "File B creation result {:?}", result);

    let meta_a = fs.metadata(&path_a);
    let meta_b = fs.metadata(&path_b);

    let accessed_a = meta_a.accessed().unwrap();
